...
- [ ] Abre com as equipes **recolhidas** (só cabeçalho: equipe + cobertura + **!** onde há vaga/troca).
- [ ] Tocar numa equipe **expande**; tocar de novo recolhe.
- [ ] Botão **"Expandir tudo / Recolher tudo"** funciona.
- [ ] A escolha **fica salva** (recarrega a página e volta do mesmo jeito).
- [ ] **Modo Ver** (padrão): cada pessoa em 1 linha, status pela **cor da borda** (verde/âmbar/vermelho), check quem fez presença, **!** onde há troca.
- [ ] Botão **"Editar"** revela os controles (precisa −N+, não se aplica, lixeira, escalar, aprovar/recusar troca) e também fica salvo.
- [ ] Líder com 1 equipe: já abre expandida.

### Pessoas — filtro por equipe (item 8)
- [ ] Chips de equipe no topo do diretório; **1 por vez** filtra a lista.
- [ ] "Todas" volta pra lista completa; cabeçalho mostra a equipe + contagem.
- [ ] Sumiu o sort "Mais equipes" (ficou A–Z / Recentes).

### Apelido (item 7)  ·  ⚠️ mexeu no banco (migração aditiva — ver seção no fim)
- [ ] **Perfil → linha "Apelido"**: adicionar/editar seu apelido salva e aparece toast.
- [ ] Cabeçalho do Perfil mostra **apelido grande + nome completo pequeno**.
- [ ] No **Escalar** (líder escolhendo): candidato aparece como **apelido · nome real**.
- [ ] No **diretório de Pessoas**: idem (apelido em destaque + nome ao lado).

### Criar evento (item 9)  ·  admin → Criar evento
- [ ] Tudo num **card só**: modelos no topo (cards com ícone + contagem) → infos → "Equipes que vão servir".
- [ ] Acabou a sensação de "box das infos + equipes soltas embaixo".
- [ ] Tocar num modelo preenche título/hora/local/equipes.

### Performance percebida (skeletons)
- [ ] Ao navegar entre abas, aparece um **esqueleto pulsando** enquanto carrega (em vez de tela travada).
