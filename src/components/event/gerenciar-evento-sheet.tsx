...
const dateTimeCls = cn(inputCls, "text-[15px] font-bold tabular-nums");

type Profile = { id: string; name: string; avatarUrl: string | null };

/**
 * Painel "Ajustes" (admin) — aba dentro do MESMO sheet da escala, não sheet
 * por cima. Edita data/hora/local/chegada, define responsável e arquiva/exclui.
 */
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 border-t border-border/60 pt-3">
        ...
        <label className="block">
          ...
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className={dateTimeCls} />
        </label>
        <label className="block">
          <span className="mb-1 block text-sm font-medium">Hora</span>
          <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className={dateTimeCls} />
        </label>
        <label className="block">
          <span className="mb-1 block text-sm font-medium">Término (opcional)</span>
          <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className={dateTimeCls} />
        </label>
        <label className="block">
          <span className="mb-1 block text-sm font-medium">Chegada da equipe</span>
          <input type="time" value={call} onChange={(e) => setCall(e.target.value)} className={dateTimeCls} />
        </label>
        <label className="block">
